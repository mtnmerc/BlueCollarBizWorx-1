...
function authenticateGPT(req: any, res: any, next: any) {
  console.log('=== GPT AUTHENTICATION EXECUTING ===');
...
export async function registerCleanRoutes(app: Express): Promise<Server> {
  ...
  // =============================================================================
  // CLEAN GPT ROUTES - ISOLATED AND PRIORITY
  // =============================================================================
  
  app.get('/api/gpt/estimates', authenticateGPT, async (req: any, res: any) => {
    console.log('=== CLEAN GPT ESTIMATES ROUTE EXECUTING ===');
    ...
    try {
      ...
      console.log(`CLEAN ROUTE: Returning ${formattedEstimates.length} formatted estimates`);
      ...
      console.error('Clean GPT Estimates error:', error);
  ...
  app.get('/api/gpt/invoices', authenticateGPT, async (req: any, res: any) => {
    console.log('=== CLEAN GPT INVOICES ROUTE EXECUTING ===');
    ...
    try {
      ...
      console.log(`CLEAN ROUTE: Returning ${formattedInvoices.length} formatted invoices`);
      ...
      console.error('Clean GPT Invoices error:', error);
  ...
  app.get('/api/gpt/clients', authenticateGPT, async (req: any, res: any) => {
    console.log('=== CLEAN GPT CLIENTS ROUTE EXECUTING ===');
    
    try {
      const business = req.business;
      const clientsList = await storage.getClientsByBusiness(business.id);
      
      console.log(`CLEAN ROUTE: Returning ${clientsList.length} clients`);
      ...
      console.error('Clean GPT Clients error:', error);
  ...
  app.post('/api/gpt/clients', authenticateGPT, async (req: any, res: any) => {
    console.log('=== CLEAN GPT CREATE CLIENT ROUTE EXECUTING ===');
    ...
    try {
      ...
      console.log(`CLEAN ROUTE: Created client ${newClient.name}`);
      ...
      console.error('Clean GPT Create Client error:', error);
