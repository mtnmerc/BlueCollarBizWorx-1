...
export function registerGPTRoutes(app: Express) {
  ...
  app.post('/api/gpt/estimates/:id/convert-to-invoice', authenticateGPT, async (req: any, res: any) => {
    ...
    try {
      ...
      const newInvoice = await storage.convertEstimateToInvoice(estimateId);
      
      console.log('GPT FINAL: Converted estimate', estimateId, 'to invoice', newInvoice.id);
  ...
  // GPT GET INVOICE STATS - Schema compliant
  app.get('/api/gpt/invoices/stats', authenticateGPT, async (req: any, res: any) => {
    ...
    try {
      ...
      // Get invoice statistics using database queries
      ...
      const unpaidInvoices = allInvoices.filter(inv => inv.status !== 'paid' && inv.status !== 'cancelled').length;
      ...
      for (let i = 5; i >= 0; i--) {
        ...
        const monthRevenue = monthlyInvoices.reduce((sum, inv) => sum + parseFloat(inv.total || '0'), 0);
      ...
      console.log('GPT FINAL: Returning invoice stats');
      
      res.json({
        success: true,
        data: stats,
        message: `Invoice statistics for ${business.name}`,
        businessVerification: {
          businessName: business.name,
          businessId: business.id,
          dataSource: "AUTHENTIC_DATABASE",
